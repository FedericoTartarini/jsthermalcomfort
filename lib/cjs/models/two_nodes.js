...
exports.roundArray = exports.two_nodes_array = exports.two_nodes = void 0;
const utilities_js_1 = require("../utilities/utilities.js");
const p_sat_torr_js_1 = require("../psychrometrics/p_sat_torr.js");
/**
 ...
 *
 */
function two_nodes(tdb, tr, v, rh, met, clo, wme = 0, body_surface_area = 1.8258, p_atmospheric = 101325, body_position = "standing", max_skin_blood_flow = 90, kwargs = {}) {
    ...
    if (joint_kwargs.calculate_ce) {
        const ce_set_result = calculate_two_nodes(tdb, tr, v, met, clo, vapor_pressure, wme, body_surface_area, p_atmospheric, body_position, max_skin_blood_flow, { calculate_ce: true });
        return {
            set: ce_set_result.set,
        };
    }
    const result = calculate_two_nodes(tdb, tr, v, met, clo, vapor_pressure, wme, body_surface_area, p_atmospheric, body_position, max_skin_blood_flow, joint_kwargs);
    if (joint_kwargs.round) {
        return {
            e_skin: (0, utilities_js_1.round)(result.eSkin, 1),
            e_rsw: (0, utilities_js_1.round)(result.eRsw, 1),
            e_max: (0, utilities_js_1.round)(result.eMax, 1),
            q_sensible: (0, utilities_js_1.round)(result.qSensible, 1),
            q_skin: (0, utilities_js_1.round)(result.qSkin, 1),
            q_res: (0, utilities_js_1.round)(result.qRes, 1),
            t_core: (0, utilities_js_1.round)(result.tCore, 1),
            t_skin: (0, utilities_js_1.round)(result.tSkin, 1),
            m_bl: (0, utilities_js_1.round)(result.mBl, 1),
            m_rsw: (0, utilities_js_1.round)(result.mRsw, 1),
            w: (0, utilities_js_1.round)(result.w, 1),
            w_max: (0, utilities_js_1.round)(result.wMax, 1),
            set: (0, utilities_js_1.round)(result.set, 1),
            et: (0, utilities_js_1.round)(result.et, 1),
            pmv_gagge: (0, utilities_js_1.round)(result.pmvGagge, 1),
            pmv_set: (0, utilities_js_1.round)(result.pmvSet, 1),
            disc: (0, utilities_js_1.round)(result.disc, 1),
            t_sens: (0, utilities_js_1.round)(result.tSens, 1),
        };
    }
    return result;
}
exports.two_nodes = two_nodes;
const kClo = 0.25;
const bodyWeight = 70;
 ...
 *
 */
function two_nodes_array(tdbArray, trArray, vArray, rhArray, metArray, cloArray, wmeArray, bodySurfaceArray, pAtmArray, bodyPositionArray, maxSkinBloodFlowArray, kwargs = {}) {
    const defaults_kwargs = {
        calculate_ce: false,
        round: true,
        max_sweating: 500,
    ...
    let joint_kwargs = Object.assign(defaults_kwargs, kwargs);
    const vaporPressureArray = rhArray.map((_rh, index) => cal_vapor_pressure(tdbArray[index], _rh));
    ...
    if (joint_kwargs.calculate_ce) {
        ...
        for (let index = 0; index < tdbArray.length; index++) {
            const ceSetResult = calculate_two_nodes(tdbArray[index], trArray[index], vArray[index], metArray[index], cloArray[index], vaporPressureArray[index], wmeArray[index], bodySurfaceArray[index], pAtmArray[index], bodyPositionArray[index], maxSkinBloodFlowArray[index], { calculate_ce: true });
    ...
    for (let index = 0; index < tdbArray.length; index++) {
        const result = calculate_two_nodes(tdbArray[index], trArray[index], vArray[index], metArray[index], cloArray[index], vaporPressureArray[index], wmeArray[index], bodySurfaceArray[index], pAtmArray[index], bodyPositionArray[index], maxSkinBloodFlowArray[index], joint_kwargs);
...
exports.two_nodes_array = two_nodes_array;
/**
...
function roundArray(array, precision) {
    for (let index = 0; index < array.length; ++index) {
        array[index] = (0, utilities_js_1.round)(array[index], precision);
    }
    return array;
}
...
function cal_vapor_pressure(tdb, rh) {
    return (rh * (0, p_sat_torr_js_1.p_sat_torr)(tdb)) / 100;
}
/**
 ...
 *
 */
function calculate_two_nodes(tdb, tr, v, met, clo, vaporPressure, wme, bodySurfaceArea, pAtmospheric, bodyPosition, maxSkinBloodFlow, kwargs) {
    ...
    while (nSimulation < lengthTimeSimulation) {
        ...
        while (!tcConverged) {
            if (bodyPosition === "sitting") {
                hR = 4.0 * 0.95 * sbc * Math.pow(((tCl + tr) / 2.0 + 273.15), 3.0) * 0.7;
            }
            else {
                hR = 4.0 * 0.95 * sbc * Math.pow(((tCl + tr) / 2.0 + 273.15), 3.0) * 0.73;
            }
            hT = hR + hCc;
            rA = 1.0 / (fACl * hT);
