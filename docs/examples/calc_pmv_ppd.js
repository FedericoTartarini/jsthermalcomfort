...
import { v_relative, clo_dynamic, met_typical_tasks, clo_individual_garments} from "../../src/utilities/utilities.js";
...
let icl = garments.reduce((acc, item) => acc + clo_individual_garments[item], 0);
...
const results_ip = pmv_ppd(77, 77, 0.6, 50, 1.1, 0.5, undefined, undefined, {units: "IP"});
console.log(results_ip)

//------------------------------------------------------------------------------------------
//If you want you can also pass an array of inputs
import {pmv_ppd_array} from "../../src/models/pmv_ppd.js";
import {v_relative_array, clo_dynamic_array} from "../../src/utilities/utilities.js";
...
const results_pmv_ppd_array = pmv_ppd_array(tdb_array, tr_array, v_rel, rh_array, met_array, clo_d, undefined, "ASHRAE", {units: "SI"});
...
pmv_ppd_array(tdb_test, tr_test, v_rel_test, rh_test, met_test, clo_d_test_ASHRAE, undefined, "ASHRAE");
const endAshrae = new Date().getTime();
console.log("ASHRAE PMV: " + (endAshrae - startAshrae) / 1000);

// ISO PMV
const startISO = new Date().getTime();
pmv_ppd_array(tdb_test, tr_test, v_rel_test, rh_test, met_test, clo_d_test_ISO, undefined, "ISO");
