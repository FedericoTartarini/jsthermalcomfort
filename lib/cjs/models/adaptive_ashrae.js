...
exports.adaptive_ashrae_array = exports.adaptive_ashrae = void 0;
 ...
 */
function adaptive_ashrae(tdb, tr, t_running_mean, v, units = "SI", limit_inputs = true) {
    ...
    const ce = (0, adaptive_en_js_1.get_ce)(v, to);
    // Relation between comfort and outdoor temperature
    let t_cmf = 0.31 * t_running_mean + 17.8;
    if (limit_inputs) {
        const { tdb: tdb_valid, tr: tr_valid, v: v_valid, } = (0, utilities_js_1.check_standard_compliance_array)(standard, {
            tdb: [tdb],
            tr: [tr],
            v: [v],
        });
        const trm_valid = t_running_mean >= 10.0 && t_running_mean <= 33.5 ? t_running_mean : NaN;
    ...
    t_cmf = (0, utilities_js_1.round)(t_cmf, 1);
...
exports.adaptive_ashrae = adaptive_ashrae;
/**
 ...
 * // if the running mean temperature is higher than 10°C
 */
function adaptive_ashrae_array(tdb, tr, t_running_mean, v, units = "SI", limit_inputs = true) {
    const standard = "ASHRAE";
    if (units === "IP") {
        ({
            tdb,
    ...
    // calculate cooling effect (ce) of elevated air speed when top > 25 degC.
    const ce = v.map((_v, index) => (0, adaptive_en_js_1.get_ce)(_v, to[index]));
    // Relation between comfort and outdoor temperature
    ...
    const acceptability_80 = tmp_cmf_80_low.map((_low, index) => _low <= to[index] && to[index] <= tmp_cmf_80_up[index]);
    const acceptability_90 = tmp_cmf_90_low.map((_low, index) => _low <= to[index] && to[index] <= tmp_cmf_90_up[index]);
    if (units === "IP") {
        ({
            tmp_cmf: t_cmf,
            tmp_cmf_80_low,
...
exports.adaptive_ashrae_array = adaptive_ashrae_array;
