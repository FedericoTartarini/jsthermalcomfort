...
exports.t_mrt_array = exports.t_mrt = void 0;
 ...
 * @returns {number}
 */
function t_mrt(tg, tdb, v, d = 0.15, emissivity = 0.95, standard = "Mixed Convection") {
    standard = standard.toLowerCase();
    let tr = 0;
    if (standard === "mixed convection") {
        tr = get_tr_mixed_convection(tg, tdb, v, d, emissivity);
        return d >= 0.04 && d <= 0.15 ? (0, utilities_js_1.round)(tr, 1) : NaN;
    }
    if (standard === "iso") {
        return get_tr_iso(tg, tdb, v, d, emissivity);
    }
    throw new Error("No standard found, please choose standard from ISO and Mixed Convectioin");
}
exports.t_mrt = t_mrt;
/**
 * Converts globe temperature reading into mean radiant temperature in accordance with either the Mixed Convection
 ...
 * @returns {number[]}
 */
function t_mrt_array(tg, tdb, v, d, emissivity, standard = "Mixed Convection") {
    ...
    if (standard === "mixed convection") {
        ...
        const d_valid = (0, utilities_js_1.valid_range)(d, [0.04, 0.15]);
        const trResult = d_valid.map((_d_valid, i) => !isNaN(_d_valid) ? (0, utilities_js_1.round)(tr_array[i], 1) : NaN);
        return trResult;
    }
    if (standard === "iso") {
        tr_array = tg.map((_tg, i) => {
...
exports.t_mrt_array = t_mrt_array;
function get_tr_mixed_convection(_tg, _tdb, _v, _d, _emissivity) {
    ...
    const tr = Math.pow(Math.pow(_tg + 273.15, 4) -
...
function get_tr_iso(_tg, _tdb, _v, _d, _emissivity) {
    ...
    const tr = Math.pow(Math.pow(_tg, 4) + (h * (_tg - _tdb)) / (_emissivity * (5.67 * Math.pow(10, -8))), 0.25) - c_to_k;
    return (0, utilities_js_1.round)(tr, 1);
}
