...
- Commit messages: `type(scope): summary`, scope the model or module touched, `!` after the scope for a breaking change, `type(#issue)` only when a GitHub issue exists; semantic-release on `main`.
