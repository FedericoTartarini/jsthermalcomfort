...
exports.t_o_array = exports.t_o = void 0;
/**
 ...
 */
function t_o(tdb, tr, v, standard = "ISO") {
    if (v < 0) {
 ...
 * @returns {number[]} operative temperature [C]
 */
function t_o_array(tdb, tr, v, standard = "ISO") {
    if (tdb.length !== tr.length || tr.length !== v.length) {
        throw new Error("Provided arrays are not the same lengths");
    }
    if (standard === "ISO")
        return tdb.map((_tdb, index) => _t_o_calculation_iso(_tdb, tr[index], v[index]));
    if (standard === "ASHRAE")
        return tdb.map((_tdb, index) => _t_o_calculation_ashrae(_tdb, tr[index], v[index]));
    throw new Error("standard must be one of ISO or ASHRAE");
}
exports.t_o_array = t_o_array;
/**
