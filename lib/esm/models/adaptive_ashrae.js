import { round, check_standard_compliance_array, units_converter, valid_range, units_converter_array, } from "../utilities/utilities.js";
 ...
 */
export function adaptive_ashrae(tdb, tr, t_running_mean, v, units = "SI", limit_inputs = true) {
    ...
    if (limit_inputs) {
        const { tdb: tdb_valid, tr: tr_valid, v: v_valid, } = check_standard_compliance_array(standard, {
            tdb: [tdb],
            tr: [tr],
            v: [v],
        });
 ...
 * // if the running mean temperature is higher than 10°C
 */
export function adaptive_ashrae_array(tdb, tr, t_running_mean, v, units = "SI", limit_inputs = true) {
    const standard = "ASHRAE";
    if (units === "IP") {
        ({
            tdb,
