 ...
 * @returns PET or energy balance.
 */
  function solve_pet(t_arr,
    _tdb,
    _tr,
    _v=0.1,
    _rh=50,
    ...
    const met_female =
        ...
        (1.0 + 0.004 * (30.0 - age) + 0.018 * (height * 100.0 / Math.pow(weight, 1.0 / 3.0) - 42.1));
    const met_male =
        3.45 *
        Math.pow(weight, 0.75) *
        (1.0 + 0.004 * (30.0 - age) + 0.01 * (height * 100.0 / Math.pow(weight, 1.0 / 3.0) - 43.4));
    ...
    if (!actual_environment) { // mode=False means we are calculating the PET
