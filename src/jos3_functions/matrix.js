...
export function vessel_blood_flow(
  ...
  bf_art.set(
    [5],
    math.sum(xbf.subset(math.index(math.range(5, 8)))) + bf_ava_hand,
  );
  ...
  bf_art.set(
    [6],
    math.sum(xbf.subset(math.index(math.range(6, 8)))) + bf_ava_hand,
  );
  ...
  bf_art.set(
    [8],
    math.sum(xbf.subset(math.index(math.range(8, 11)))) + bf_ava_hand,
  );
  ...
  bf_art.set(
    [9],
    math.sum(xbf.subset(math.index(math.range(9, 11)))) + bf_ava_hand,
  );
  ...
  bf_art.set(
    [11],
    math.sum(xbf.subset(math.index(math.range(11, 14)))) + bf_ava_foot,
  );
  ...
  bf_art.set(
    [12],
    math.sum(xbf.subset(math.index(math.range(12, 14)))) + bf_ava_foot,
  );
  ...
  bf_art.set(
    [14],
    math.sum(xbf.subset(math.index(math.range(14, 17)))) + bf_ava_foot,
  );
  ...
  bf_art.set(
    [15],
    math.sum(xbf.subset(math.index(math.range(15, 17)))) + bf_ava_foot,
  );
...
export function local_arr(
  ...
  let bf_local = math.zeros(NUM_NODES, NUM_NODES);
  ...
  for (let i = 0; i < BODY_NAMES.length; i++) {
    ...
    // Common
    bf_local.set(
      [index_of["core"], index_of["artery"]],
      1.067 * bf_core.get([i]),
    ); // art to cr

    bf_local.set(
      [index_of["skin"], index_of["artery"]],
      1.067 * bf_skin.get([i]),
    ); // art to sk

    bf_local.set(
      [index_of["vein"], index_of["core"]],
      1.067 * bf_core.get([i]),
    ); // vein to cr

    bf_local.set(
      [index_of["vein"], index_of["skin"]],
      1.067 * bf_skin.get([i]),
    ); // vein to sk
    ...
    if (index_of["muscle"] !== null) {
      bf_local.set(
        [index_of["muscle"], index_of["artery"]],
        1.067 * bf_muscle.get([i]),
      ); // art to ms

      bf_local.set(
        [index_of["vein"], index_of["muscle"]],
        1.067 * bf_muscle.get([i]),
      ); // vein to ms
    }
    ...
    if (index_of["fat"] !== null) {
      bf_local.set(
        [index_of["fat"], index_of["artery"]],
        1.067 * bf_fat.get([i]),
      ); // art to ft

      bf_local.set(
        [index_of["vein"], index_of["fat"]],
        1.067 * bf_fat.get([i]),
      ); // vein to ft
    }
    ...
    if (i === 7 || i === 10) {
      bf_local.set(
        [index_of["sfvein"], index_of["artery"]],
        1.067 * bf_ava_hand,
      ); // art to sfv
    }
...
export function whole_body(bf_art, bf_vein, bf_ava_hand, bf_ava_foot) {
  let arr83 = math.zeros(NUM_NODES, NUM_NODES);

  const flow = (up, down, bloodflow) =>
    arr83.set([down, up], bloodflow * 1.067); // Coefficient = 1.067 [Wh/L.K] Change units [L/h] to [W/K]
