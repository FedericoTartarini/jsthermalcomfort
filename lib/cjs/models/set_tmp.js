...
exports.set_tmp_array = exports.set_tmp = void 0;
const two_nodes_js_1 = require("../models/two_nodes.js");
 ...
 */
function set_tmp(tdb, tr, v, rh, met, clo, wme = 0, body_surface_area, p_atm, body_position = "standing", units = "SI", limit_inputs = true, kwargs = {}) {
    ...
    if (units === "IP") {
        const unit_convert = (0, utilities_js_1.units_converter)({
            tdb: tdb,
            tr: tr,
            v: v,
    ...
    let set_tmp = (0, two_nodes_js_1.two_nodes)(tdb, tr, v, rh, met, clo, wme, body_surface_area, p_atm, body_position, undefined, { round: false, calculate_ce: joint_kwargs.calculate_ce }).set;
    if (units === "IP") {
        ({ tmp: set_tmp } = (0, utilities_js_1.units_converter)({ tmp: set_tmp }, "SI"));
    }
    if (limit_inputs) {
        const { tdb: tdb_valid, tr: tr_valid, v: v_valid, met: met_valid, clo: clo_valid, } = (0, utilities_js_1.check_standard_compliance_array)("ASHRAE", {
            tdb: [tdb],
            tr: [tr],
            v: [v],
            met: [met],
    ...
    if (joint_kwargs.round) {
        set_tmp = (0, utilities_js_1.round)(set_tmp, 1);
    }
    return { set: set_tmp };
}
exports.set_tmp = set_tmp;
/**
 ...
 */
function set_tmp_array(tdbArray, trArray, vArray, rhArray, metArray, cloArray, wmeArray, bodySurfaceArray, pAtmArray, bodyPositionArray, units = "SI", limit_inputs = true, kwargs = {}) {
    const defaults_kwargs = {
        calculate_ce: false,
        round: true,
    };
    ...
    if (units === "IP") {
        const unit_convert = (0, utilities_js_1.units_converter_array)({
            tdb: tdbArray,
            tr: trArray,
            v: vArray,
    ...
    let setArray = (0, two_nodes_js_1.two_nodes_array)(tdbArray, trArray, vArray, rhArray, metArray, cloArray, wmeArray, bodySurfaceArray, pAtmArray, bodyPositionArray, undefined, { round: false, calculate_ce: joint_kwargs.calculate_ce }).set;
    if (units === "IP") {
        const convertedSet = (0, utilities_js_1.units_converter_array)({
            tmp: setArray,
        }, "SI");
        setArray = convertedSet.tmp;
    }
    if (limit_inputs) {
        const { tdb: tdbValid, tr: trValid, v: vValid, met: metValid, clo: cloValid, } = (0, utilities_js_1.check_standard_compliance_array)("ASHRAE", {
            tdb: tdbArray,
            tr: trArray,
            v: vArray,
            met: metArray,
    ...
    if (joint_kwargs.round) {
        return (0, two_nodes_js_1.roundArray)(setArray, 1);
    }
    return setArray;
}
exports.set_tmp_array = set_tmp_array;
