...
export function conv_coef(
  ...
  tdb = math.multiply(math.ones(17), JOS3Defaults.dry_bulb_air_temperature),
  t_skin = math.multiply(math.ones(17), JOS3Defaults.skin_temperature),
) {
  ...
  const natural_convection = (posture, tdb, t_skin) => {
    ...
    switch (posture) {
      ...
      case "supine":
        ...
        hc_natural = math.dotMultiply(
          hc_a,
          math.dotPow(math.abs(math.subtract(tdb, t_skin)), hc_b),
        );
        break;
    }
  ...
  return v.map((z, idx) =>
    z < 0.2 ? hc_natural.get(idx) : hc_forced.get(idx),
  );
}
