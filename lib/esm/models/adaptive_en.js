...
import { units_converter, round, units_converter_array, valid_range, } from "../utilities/utilities.js";
/**
 ...
 */
export function adaptive_en(tdb, tr, t_running_mean, v, units = "SI", limit_inputs = true) {
    const standard = "ISO";
 ...
 * // if the running mean temperature is between 10 °C and 30 °C
 */
export function adaptive_en_array(tdb, tr, t_running_mean, v, units = "SI", limit_inputs = true) {
    const standard = "ISO";
    if (units.toLowerCase() == "ip") {
        ({
            tdb,
    ...
    for (let i = 0; i < t_cmf_i_lower.length; ++i) {
        t_cmf_i_lower[i] = round(t_cmf_i_lower[i], 1);
    }
    for (let i = 0; i < t_cmf_ii_lower.length; ++i) {
        t_cmf_ii_lower[i] = round(t_cmf_ii_lower[i], 1);
    }
    for (let i = 0; i < t_cmf_iii_lower.length; ++i) {
        t_cmf_iii_lower[i] = round(t_cmf_iii_lower[i], 1);
    }
    for (let i = 0; i < t_cmf_i_upper.length; ++i) {
        t_cmf_i_upper[i] = round(t_cmf_i_upper[i], 1);
    }
    for (let i = 0; i < t_cmf_ii_upper.length; ++i) {
        t_cmf_ii_upper[i] = round(t_cmf_ii_upper[i], 1);
    }
    for (let i = 0; i < t_cmf_iii_upper.length; ++i) {
        t_cmf_iii_upper[i] = round(t_cmf_iii_upper[i], 1);
    }
    return {
        tmp_cmf: t_cmf,
        acceptability_cat_i: acceptability_i,
