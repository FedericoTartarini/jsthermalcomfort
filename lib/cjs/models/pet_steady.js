 ...
 */
function pet_steady(tdb, tr, v, rh, met, clo, p_atm = 1013.25, position = 1, age = 23, sex = 1, weight = 75, height = 1.8, wme = 0) {
...
function pet_fc(_t_stable) {
    ...
    return (0, utilities_js_2.round)(result[0], 2);
}
/**
...
function solve_pet(t_arr, _tdb, _tr, _v = 0.1, _rh = 50, _met = 80, _clo = 0.9, actual_environment = false, p_atm = 1013.25, position = 1, age = 23, sex = 1, weight = 75, height = 1.8, wme = 0) {
    ...
    const a_dubois = (0, utilities_js_1.body_surface_area)(weight, height);
    ...
    const f_a_cl = (173.51 * _clo - 2.36 - 100.76 * _clo * _clo + 19.28 * Math.pow(_clo, 3.0)) / 100;
    ...
    if (actual_environment) {
        ...
        vpa = ((_rh / 100.0) * (0, p_sat_js_1.p_sat)(_tdb)) / 100;
    }
    ...
    const vpexp = (0, p_sat_js_1.p_sat)(t_exp) / 100; // Latent heat energy loss [hPa]
    ...
    const p_v_sk = (0, p_sat_js_1.p_sat)(t_arr[1]) / 100; // hPa
    ...
    const r_bare = (a_r_eff *
        ...
        e_skin *
        sbc *
        (Math.pow((_tr + 273.15), 4.0) - Math.pow((t_arr[1] + 273.15), 4.0))) /
        a_dubois;
    // ... for clothed area
    const r_clo = (f_eff *
        a_clo *
        e_clo *
        sbc *
        (Math.pow((_tr + 273.15), 4.0) - Math.pow((t_arr[2] + 273.15), 4.0))) /
        a_dubois;
