...
export function skin_blood_flow(
  ...
  let bf_skin = math.dotMultiply(
    math.dotMultiply(
      math.dotDivide(
        math.add(
          1,
          math.multiply(math.dotMultiply(skin_dilat, sd_dilat), sig_dilat),
        ),
        math.add(
          1,
          math.multiply(math.dotMultiply(skin_stric, sd_stric), sig_stric),
        ),
      ),
      bfb_sk,
    ),
    math.dotPow(2, math.divide(err_sk, 6)),
  );
