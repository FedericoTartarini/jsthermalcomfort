 ...
 * // if the running mean temperature is between 10 °C and 30 °C
 */
export function adaptive_en_array(
  tdb,
  tr,
  t_running_mean,
  v,
  ...
  if (limit_inputs) {
    ...
    trm_valid.forEach((_trm_valid, index) => {
      if (isNaN(_trm_valid)) t_cmf[index] = NaN;
    });
  }
  ...
  const acceptability_ii = to.map(
    (_to, index) =>
      t_cmf_ii_lower[index] <= _to && _to <= t_cmf_ii_upper[index],
  );
  const acceptability_iii = to.map(
    (_to, index) =>
      t_cmf_iii_lower[index] <= _to && _to <= t_cmf_iii_upper[index],
  );
