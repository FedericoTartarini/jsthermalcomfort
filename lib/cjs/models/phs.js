 ...
 */
function phs(tdb, tr, v, rh, met, clo, posture, wme = 0, kwargs = {}) {
    ...
    const warnings = (0, utilities_js_1.check_standard_compliance)("ISO7933", {
        tdb,
        tr,
        v,
        rh,
        met: met * (0, utilities_js_1.body_surface_area)(joint_kwargs.weight, joint_kwargs.height),
        clo,
    });
    warnings.forEach((warning) => console.warn(warning));
    const p_a = (((0, p_sat_js_1.p_sat)(tdb) / 1000) * rh) / 100;
    const variables_for_loop = _calculate_variables_for_loop(v, met, posture, clo, tdb, p_a, joint_kwargs);
    const result = _phs_loop(tdb, tr, v, met, clo, wme, p_a, joint_kwargs, variables_for_loop);
    if (joint_kwargs.round) {
        return {
            t_re: (0, utilities_js_1.round)(result.t_re, 1),
            t_sk: (0, utilities_js_1.round)(result.t_sk, 1),
            t_cr: (0, utilities_js_1.round)(result.t_cr, 1),
            t_cr_eq: (0, utilities_js_1.round)(result.t_cr_eq, 1),
            t_sk_t_cr_wg: (0, utilities_js_1.round)(result.t_sk_t_cr_wg, 2),
            d_lim_loss_50: (0, utilities_js_1.round)(result.d_lim_loss_50, 1),
            d_lim_loss_95: (0, utilities_js_1.round)(result.d_lim_loss_95, 1),
            d_lim_t_re: (0, utilities_js_1.round)(result.d_lim_t_re, 1),
            water_loss_watt: (0, utilities_js_1.round)(result.water_loss_watt, 1),
            water_loss: (0, utilities_js_1.round)(result.water_loss, 1),
        };
    }
    return result;
}
...
function _calculate_variables_for_loop(v, met, posture, clo, tdb, p_a, kwargs) {
    ...
    const a_dubois = 0.202 * Math.pow(kwargs.weight, 0.425) * Math.pow(kwargs.height, 0.725);
    ...
    let corr_ia = Math.exp((0.047 * v_r - 0.472) * v_r + (0.117 * w_a_ux - 0.342) * w_a_ux);
    corr_ia = Math.min(1, corr_ia);
    const corr_tot = clo <= 0.6 ? ((0.6 - clo) * corr_ia + clo * corr_cl) / 0.6 : corr_cl;
    const fcl = 1 + 0.3 * clo;
    const i_a_st = 0.111;
...
function _phs_loop(tdb, tr, v, met, clo, wme, p_a, kwargs, variables) {
    let { sw_max, w_max, fcl, i_cl_dyn, r_t_dyn, c_res, e_res, hc_dyn, aux_r, f_cl_r, a_dubois, sp_heat, d_lim_t_re, d_lim_loss_50, d_lim_loss_95, d_max_50, d_max_95, const_t_eq, const_t_sk, const_sw, } = variables;
    ...
    for (let time = 1; time <= duration; time++) {
        ...
        let t_sk_eq_cl = 12.165 + 0.02017 * tdb + 0.04361 * tr + 0.19354 * p_a - 0.25315 * v;
        ...
        while (true) {
            ...
            h_r =
                (f_cl_r * aux_r * (Math.pow((t_cl + 273), 4) - Math.pow((tr + 273), 4))) / (t_cl - tr);
        ...
        let e_req = met - d_stored_eq - wme - c_res - e_res - convection - radiation;
        ...
        if (e_req <= 0) {
            ...
            let e_v_eff = 1 - Math.pow(w_req, 2) / 2;
            if (w_req > 1) {
