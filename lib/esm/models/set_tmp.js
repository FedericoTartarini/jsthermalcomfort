...
import { check_standard_compliance_array, round, units_converter, units_converter_array, } from "../utilities/utilities.js";
/**
 ...
 */
export function set_tmp(tdb, tr, v, rh, met, clo, wme = 0, body_surface_area, p_atm, body_position = "standing", units = "SI", limit_inputs = true, kwargs = {}) {
    ...
    let set_tmp = two_nodes(tdb, tr, v, rh, met, clo, wme, body_surface_area, p_atm, body_position, undefined, { round: false, calculate_ce: joint_kwargs.calculate_ce }).set;
    if (units === "IP") {
        ({ tmp: set_tmp } = units_converter({ tmp: set_tmp }, "SI"));
    }
    if (limit_inputs) {
        const { tdb: tdb_valid, tr: tr_valid, v: v_valid, met: met_valid, clo: clo_valid, } = check_standard_compliance_array("ASHRAE", {
            tdb: [tdb],
            tr: [tr],
            v: [v],
            met: [met],
 ...
 */
export function set_tmp_array(tdbArray, trArray, vArray, rhArray, metArray, cloArray, wmeArray, bodySurfaceArray, pAtmArray, bodyPositionArray, units = "SI", limit_inputs = true, kwargs = {}) {
    const defaults_kwargs = {
        calculate_ce: false,
        round: true,
    };
    ...
    let setArray = two_nodes_array(tdbArray, trArray, vArray, rhArray, metArray, cloArray, wmeArray, bodySurfaceArray, pAtmArray, bodyPositionArray, undefined, { round: false, calculate_ce: joint_kwargs.calculate_ce }).set;
    if (units === "IP") {
    ...
    if (limit_inputs) {
        const { tdb: tdbValid, tr: trValid, v: vValid, met: metValid, clo: cloValid, } = check_standard_compliance_array("ASHRAE", {
            tdb: tdbArray,
            tr: trArray,
            v: vArray,
            met: metArray,
