...
export function athb_array(tdb, tr, vr, rh, met, t_running_mean) {
    ...
    const pmv_res = tdb.map((tdbValue, i) => pmv_calculation(tdbValue, tr[i], vr[i], rh[i], met_adapted[i], clo_adapted[i], 0));
    ...
    return l_adapted.map((lValue, i) => round(1.484 +
        0.0276 * lValue -
