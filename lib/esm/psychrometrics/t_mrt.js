 ...
 * @returns {number[]}
 */
export function t_mrt_array(tg, tdb, v, d, emissivity, standard = "Mixed Convection") {
    ...
    if (standard === "mixed convection") {
        ...
        const trResult = d_valid.map((_d_valid, i) => !isNaN(_d_valid) ? round(tr_array[i], 1) : NaN);
        return trResult;
    }
    if (standard === "iso") {
        tr_array = tg.map((_tg, i) => {
