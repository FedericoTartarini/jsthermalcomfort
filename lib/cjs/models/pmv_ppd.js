...
exports.pmv_calculation = exports.pmv_ppd_array = exports.pmv_ppd = void 0;
 ...
 */
function pmv_ppd(tdb, tr, vr, rh, met, clo, wme = 0, standard = "ISO", kwargs = {}) {
    ...
    if (kwargs.units && kwargs.units.toUpperCase() === "IP") {
        ...
        ({ tdb, tr, vr } = (0, utilities_js_1.units_converter)({ tdb, tr, vr }, "IP"));
    }
    if (standard !== "ISO" && standard !== "ASHRAE") {
        throw new Error("PMV calculations can only be performed in compliance with ISO or ASHRAE Standards");
    }
    const { tdb: tdb_valid, tr: tr_valid, v: v_valid, met: met_valid, clo: clo_valid, } = (0, utilities_js_1.check_standard_compliance_array)(standard, {
        tdb: [tdb],
        tr: [tr],
        v: [vr],
        met: [met],
    ...
    if (standard === "ASHRAE") {
        ...
        ce = vr > 0.1 ? (0, cooling_effect_js_1.cooling_effect)(tdb, tr, vr, rh, met, clo, wme).ce : 0;
    }
    tdb = tdb - ce;
    tr = tr - ce;
    vr = ce > 0 ? 0.1 : vr;
    ...
    if (kwargs.limit_inputs) {
        const pmv_valid = standard === "ASHRAE"
            ? (0, utilities_js_1.valid_range)([pmv], [-100, 100])
            : (0, utilities_js_1.valid_range)([pmv], [-2, 2]); // this is the ISO limit
        if (isNaN(pmv) ||
    ...
    return {
        pmv: (0, utilities_js_1.round)(pmv, 2),
        ppd: (0, utilities_js_1.round)(ppd, 1),
    };
}
exports.pmv_ppd = pmv_ppd;
/**
 ...
 */
function pmv_ppd_array(tdb, tr, vr, rh, met, clo, wme, standard = "ISO", kwargs = {}) {
    const default_kwargs = {
        units: "SI",
        limit_inputs: true,
        airspeed_control: true,
    ...
    if (kwargs.units && kwargs.units === "IP") {
        // Conversion from IP to SI units
        ({ tdb, tr, vr } = (0, utilities_js_1.units_converter_array)({ tdb, tr, vr }, "IP"));
    }
    if (standard !== "ISO" && standard !== "ASHRAE") {
        throw new Error("PMV calculations can only be performed in compliance with ISO or ASHRAE Standards");
    }
    ...
    if (standard === "ASHRAE") {
        ce = vr.map((vrValue, i) => {
            //if v_r is higher than 0.1 follow methodology ASHRAE Appendix H, H3
            return vrValue > 0.1
                ? (0, cooling_effect_js_1.cooling_effect)(tdb[i], tr[i], vrValue, rh[i], met[i], clo[i], wme[i])
                    .ce
                : 0;
        });
    }
    ...
    const pmv_array = tdb.map((tdbValue, i) => pmv_calculation(tdbValue, tr[i], vr[i], rh[i], met[i], clo[i], wme[i]));
    const ppd_array = pmv_array.map((pmvValue) => {
        return (100 -
            95 *
                Math.exp(-0.03353 * Math.pow(pmvValue, 4) - 0.2179 * Math.pow(pmvValue, 2)));
    });
    // Checks that inputs are within the bounds accepted by the model if not return NaN
    if (kwargs.limit_inputs) {
        const pmv_valid = standard === "ASHRAE"
            ? (0, utilities_js_1.valid_range)(pmv_array, [-100, 100])
            : (0, utilities_js_1.valid_range)(pmv_array, [-2, 2]); // this is the ISO limit
        for (let i = 0; i < pmv_array.length; ++i) {
            if (isNaN(pmv_array[i]) ||
                isNaN(tdb_valid[i]) ||
                isNaN(tr_valid[i]) ||
                isNaN(v_valid[i]) ||
                isNaN(met_valid[i]) ||
    ...
    for (let i = 0; i < pmv_array.length; ++i) {
        pmv_array[i] = (0, utilities_js_1.round)(pmv_array[i], 2);
        ppd_array[i] = (0, utilities_js_1.round)(ppd_array[i], 1);
    }
    return {
        pmv: pmv_array,
        ppd: ppd_array,
...
exports.pmv_ppd_array = pmv_ppd_array;
/**
 * @param {number} tdb
 * @param {number} tr
 * @param {number} vr
...
exports.pmv_calculation = pmv_calculation;
