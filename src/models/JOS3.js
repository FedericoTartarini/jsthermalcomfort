...
export class JOS3 {
  ...
  constructor(
    ...
    this._bodytemp = math.multiply(
      math.ones(NUM_NODES),
      JOS3Defaults.other_body_temperature,
    );

    // Initialize environmental conditions and other factors
    // (Default values of input conditions)
    this._ta = math.multiply(
      math.ones(17),
      JOS3Defaults.dry_bulb_air_temperature,
    );
    this._tr = math.multiply(
      math.ones(17),
      JOS3Defaults.mean_radiant_temperature,
    );
  ...
  _reset_setpt(par = JOS3Defaults.physical_activity_ratio) {
    ...
    this.to = this._calculate_operative_temp_when_pmv_is_zero(
      undefined,
      undefined,
      met,
      undefined,
  ...
  _run(dtime = 60, passive = false, output = true) {
    ...
    let p_a = math.dotDivide(
      math.dotMultiply(this._ta.map(antoine), this._rh),
      100,
    );
    ...
    let shl_sk = math.dotMultiply(
      math.dotDivide(math.subtract(tsk, to), r_t),
      this._bsa,
    );
    ...
    let wlesk = math.dotDivide(
      math.add(e_sweat, math.dotMultiply(0.06, e_max)),
      2418,
    );
    ...
    arr_b = math.subset(
      arr_b,
      math.index(INDEX["skin"]),
      math.add(
        math.subset(arr_b, math.index(INDEX["skin"])),
        math.dotMultiply(math.dotDivide(1, r_t), this._bsa),
      ),
    );
    ...
    arr_q = math.subset(
      arr_q,
      math.index(INDEX["core"]),
      math.add(
        math.subset(arr_q, math.index(INDEX["core"])),
        q_thermogenesis_core,
      ),
    );
    ...
    arr_q = math.subset(
      arr_q,
      math.index(INDEX["muscle"]),
      math.add(
        math.subset(arr_q, math.index(INDEX["muscle"])),
        math.subset(q_thermogenesis_muscle, math.index(VINDEX["muscle"])),
      ),
    );

    arr_q = math.subset(
      arr_q,
      math.index(INDEX["fat"]),
      math.add(
        math.subset(arr_q, math.index(INDEX["fat"])),
        math.subset(q_thermogenesis_fat, math.index(VINDEX["fat"])),
      ),
    );
    ...
    arr_q = math.subset(
      ...
      math.index(INDEX["skin"]),
      math.add(
        math.subset(arr_q, math.index(INDEX["skin"])),
        q_thermogenesis_skin,
      ),
    );
    ...
    arr_q.set(
      [INDEX["core"][2]],
      arr_q.get([INDEX["core"][2]]) - (res_sh + res_lh),
    ); // chest core
    ...
    arr_q = math.subset(
      arr_q,
      math.index(INDEX["skin"]),
      math.subtract(math.subset(arr_q, math.index(INDEX["skin"])), e_sk),
    );
    ...
    arr_to = math.subset(
      arr_to,
      math.index(INDEX["skin"]),
      math.add(math.subset(arr_to, math.index(INDEX["skin"])), to),
    );
    ...
    let arr = math.add(
      math.add(this._bodytemp, math.dotMultiply(arr_b, arr_to)),
      arr_q,
    );
  ...
  get t_skin_mean() {
    return (
      math.sum(
        math.dotMultiply(
          math.subset(this._bodytemp, math.index(INDEX["skin"])),
